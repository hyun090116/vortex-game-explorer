...
const GameCard = ({ game, index = 0, showCartButton = true }: GameCardProps) => {
  ...
  const isOwned = isPurchased(game.id, game.title);
    ...
    >
      ...
      <div className="glass-card overflow-hidden relative">
        ...
        <div className="relative aspect-[3/4] overflow-hidden">
          ...
          {/* Quick Add Button or Owned Badge */}
          {showCartButton && (
            <div className="absolute bottom-3 left-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
              {isOwned ? (
                <Button 
                  variant="outline" 
                  className="w-full gap-2 bg-green-500/20 border-green-500/50 text-green-500 hover:bg-green-500/30" 
                  size="sm"
                  disabled
                >
